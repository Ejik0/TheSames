...
for index in df.index:
    if df.loc[index,'Country']=='United States of America':
...
#No, its seems that if a country has a lower GDP the LEABY is mych higher
...
ax = sns.violinplot(data=df, x='Country', y='LEABY',linewidth=1.50)
...
sns.set_style('whitegrid')
plt.savefig('GDP as a function Life Expectancy by country.png')
plt.show()
...
g3.set(xticks=[2000, 2002, 2005, 2007, 2010, 2012, 2015])

plt.savefig('graphs mapping Life Expectancy by country.png')
plt.show()
...
g4.set(xticks=[2000, 2002, 2005, 2007, 2010, 2012, 2015])

plt.savefig('graphs mapping GDP by country.png')
plt.show()
